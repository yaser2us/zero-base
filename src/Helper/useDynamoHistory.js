...
const useDynamoHistory = (
    ...
    const current = () =>
        currentIndex >= 0 && currentIndex < history.length ? history[currentIndex] : false;
    ...
    const insertObject = (
        ...
        if (preventDuplicates && !customIgnoreDuplicates) {
            ...
            if (existingIndex !== -1) {
                if (
                    customReplaceDuplicate !== undefined ? customReplaceDuplicate : replaceDuplicate
                ) {
                    const newArray = [...history];
        ...
        const objectType =
            previousObject && previousObject.type === "primary" ? "secondary" : "primary";
